...
MERGE (s1:Session {
  id: 'sample_session_1',
  created_at: datetime(),
  metadata: {type: 'research_session', agent: 'ResearchAgent'}
})

MERGE (s2:Session {
  id: 'sample_session_2', 
  created_at: datetime(),
  metadata: {type: 'qa_session', agent: 'QualityAssuranceAgent'}
})
...
// Count nodes by type
MATCH (n:KnowledgeEntity) RETURN 'KnowledgeEntity' as type, count(n) as count
UNION
MATCH (n:ResearchInsights) RETURN 'ResearchInsights' as type, count(n) as count
UNION  
MATCH (n:QAInsights) RETURN 'QAInsights' as type, count(n) as count
UNION
MATCH (n:Session) RETURN 'Session' as type, count(n) as count
UNION
MATCH (n:Orchestration) RETURN 'Orchestration' as type, count(n) as count;
